...
function buildSVG(title, counts, names, colors) {
  ...
  const plotAreaWidth = canvasWidth - legendWidth - leftPadding - 20;
  const plotAreaHeight = canvasHeight - titleHeight - bottomPadding;

  const centerX = leftPadding + plotAreaWidth / 2;
  ...
  // Estimate number of layers needed and radius
  const maxLayers = 100;
  const minGap = 1;
  let layers = 0, remaining = total;
  let seatR = 5; // initial guess

  // Estimate required layers based on rough fixed seatR
  ...
  // Now recalculate proper seatR based on available plot height

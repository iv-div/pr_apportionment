...
function buildSVG(title, counts, names, colors, seatR = 6, gap = 1, innerR = 28) {
  ...
  const legendEntryHeight = 20 * textScaleFactor;
  const estimatedLegendHeight = partyObjs.length * legendEntryHeight + 40 * textScaleFactor;
  const parliamentHeight = baseOuterR + innerR;

  const horizontalPadding = 40 * textScaleFactor;
  const legendWidth = 200 * textScaleFactor;
  const diagramWidth = baseOuterR * 2;

  const totalWidth = diagramWidth + legendWidth + 3 * horizontalPadding;

  const topPaddingTitle = 60 * textScaleFactor;
  const bottomPadding = 40 * textScaleFactor;
  ...
  const scaledSeatR = Math.max(1, seatR * circleScaleFactor);
  ...
  partyObjs.forEach(p => {
    ...
    for (let k = 0; k < p.c; k++) {
      ...
      const strokeWidth = isDisputed ? (0.8 * circleScaleFactor) : (0.5 * circleScaleFactor);

      svg.push(`<circle cx="${diagramCenterX + x * circleScaleFactor}" 
                        cy="${diagramCenterY - y * circleScaleFactor}" 
                        r="${scaledSeatR}" 
                        fill="${fillColor}" 
                        stroke="${strokeColor}" 
                        stroke-width="${Math.max(0.1, strokeWidth)}"/>`);
    }
  });

  svg.push(`<text x="${diagramCenterX}" y="${diagramCenterY + scaledSeatR + 20 * textScaleFactor}" text-anchor="middle" font-size="${Math.max(8, 14 * textScaleFactor)}" font-family="sans-serif">${total} seats</text>`);

  const legX = diagramWidth + 2 * horizontalPadding;
  let legYCurrent = topPaddingTitle;

  partyObjs.forEach((p) => {
    if (legYCurrent > totalHeight - bottomPadding - legendEntryHeight) return;
    const isDisputed = p.name.toLowerCase().includes("disputed mandates");
    const fillColor = p.color;
    const rectStroke = isDisputed ? "black" : "none";
    const rectSize = Math.max(5, 15 * circleScaleFactor);
    ...
    svg.push(`<text x="${legX + rectSize * 1.5}" y="${legYCurrent}" dominant-baseline="middle" font-size="${Math.max(8, 12 * textScaleFactor)}" font-family="sans-serif">${p.name} – ${p.c}</text>`);
